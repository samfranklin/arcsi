...
class ARCSIParamsObj(object):
    def __init__(self):
        ...
        debugMode = False
        scaleFactor = 1000
        interpAlgor = rsgislib.INTERP_CUBIC
...
def prepParametersObj(
    ...
    aeroProfileOption,
    atmosProfileOption,
    aeroProfileOptionImg,
    atmosProfileOptionImg,
    grdReflOption,
    surfaceAltitude,
    atmosOZoneVal,
    atmosWaterVal,
    ...
    if paramsObj.inWKTFile is not None:
        paramsObj.wktStr = rsgislib.tools.utils.read_text_file_no_new_lines(
            paramsObj.inWKTFile
        )

    # Step 1: Get the Sensor specific class from factory
    sensorFact = arcsilib.arcsiutils.ARCSISensorFactory()
    paramsObj.sensorClass = sensorFact.getSensorClassFromName(
        paramsObj.sensorStr, paramsObj.debugMode, paramsObj.inputImage
    )
    # Step 2: Read header parameters
    paramsObj.sensorClass.extractHeaderParameters(
    ...
    if paramsObj.aeroProfileOption is None:
        if paramsObj.aeroProfileOptionImg is not None:
            ...
            aeroProfileMode = int(
                rsgislib.imagecalc.get_img_band_mode_in_env(
                    paramsObj.aeroProfileOptionImg,
                    1,
                    1,
                    None,
                    paramsObj.sensorClass.lonTL,
                    paramsObj.sensorClass.lonBR,
            ...
            if aeroProfileMode == 1:
                ...
                raise ARCSIException(
                    "The aerosol profile from the input image was not recognised."
                )
    ...
    if paramsObj.atmosProfileOption is None:
        if paramsObj.atmosProfileOptionImg is not None:
            ...
            atmosProfileMode = int(
                rsgislib.imagecalc.get_img_band_mode_in_env(
                    paramsObj.atmosProfileOptionImg,
                    1,
                    1,
                    None,
                    paramsObj.sensorClass.lonTL,
            ...
            if atmosProfileMode == 1:
                ...
                raise ARCSIException(
                    "The atmosphere profile from the input image was not recognised."
                )
    ...
    if (paramsObj.outBaseName is None) or (paramsObj.outBaseName == ""):
        ...
        if not paramsObj.projAbbv is None:
            paramsObj.outBaseNameProj = (
                paramsObj.outBaseName + "_" + str(paramsObj.projAbbv)
            )
    ...
    if paramsObj.checkOutputs:
        ...
        if len(prevOutFiles) > 0:
            ...
            raise ARCSIException(
                "Output files already exist and the 'check outputs' option (--checkouts) was specified so can't continue."
            )
    ...
    for prod in paramsObj.productsStr:
        if prod == "RAD":
            ...
            if paramsObj.sensorClass.hasThermal():
                ...
                raise ARCSIException(
                    "The sensor does not have thermal bands. Check you inputs."
                )
            ...
            if paramsObj.resample2LowResImg:
                raise ARCSIException(
                    "'SHARP' product is only appropriate if lower resolution images bands are being sharpened to a higher resolution."
                )

    if paramsObj.prodsToCalc["DOSAOT"] and paramsObj.prodsToCalc["DDVAOT"]:
        raise ARCSIException(
            "You cannot specify both the DOSAOT and DDVAOT products, you must choose one or the other."
        )
    ...
    if paramsObj.needAtmModel:
        if paramsObj.aeroProfileOption is None:
            ...
            paramsObj.aeroProfile = Py6S.AeroProfile.PredefinedType(
                Py6S.AeroProfile.NoAerosols
            )
        ...
        if paramsObj.atmosProfileOption is None:
            ...
            paramsObj.atmosProfile = Py6S.AtmosProfile.PredefinedType(
                Py6S.AtmosProfile.NoGaseousAbsorption
            )
        ...
        if paramsObj.grdReflOption is None:
            ...
            paramsObj.grdRefl = Py6S.GroundReflectance.HomogeneousLambertian(
                Py6S.GroundReflectance.GreenVegetation
            )
        elif paramsObj.grdReflOption == "ClearWater":
            paramsObj.grdRefl = Py6S.GroundReflectance.HomogeneousLambertian(
                Py6S.GroundReflectance.ClearWater
            )
        elif paramsObj.grdReflOption == "Sand":
            paramsObj.grdRefl = Py6S.GroundReflectance.HomogeneousLambertian(
                Py6S.GroundReflectance.Sand
            )
        elif paramsObj.grdReflOption == "LakeWater":
            paramsObj.grdRefl = Py6S.GroundReflectance.HomogeneousLambertian(
                Py6S.GroundReflectance.LakeWater
            )
        elif paramsObj.grdReflOption == "BRDFHapke":
            paramsObj.grdRefl = Py6S.GroundReflectance.HomogeneousHapke(
                0.101, -0.263, 0.589, 0.046
            )
    ...
    if demNoDataUsrVal is not None:
        ...
        paramsObj.demNoDataVal = rsgislib.imageutils.get_img_no_data_value(
            paramsObj.demFile
        )
        if paramsObj.demNoDataVal is None:
            raise ARCSIException(
                "A no data value for the inputted DEM has not been defined - cannot continue without a no data value. A no data value can be define using the --demnodata option."
            )
...
def resampleBands(paramsObj):
    ...
    if paramsObj.sensorClass.inImgsDiffRes():
        ...
        if paramsObj.interpAlgorResample == "near":
            paramsObj.interpAlgorResample = rsgislib.INTERP_NEAREST_NEIGHBOUR
        paramsObj.sensorClass.resampleImgRes(
            paramsObj.outFilePath,
            paramsObj.resample2LowResImg,
            paramsObj.interpAlgorResample,
            False,
        )
...
def createValidMaskViewAngle(paramsObj):
    ...
    paramsObj.viewAngleImg = os.path.join(
        paramsObj.outFilePath,
        paramsObj.outBaseName + "_viewangle" + paramsObj.outFormatExt,
    )
    paramsObj.validMaskImage = paramsObj.sensorClass.generateValidImageDataMask(
        paramsObj.outFilePath, outName, paramsObj.viewAngleImg, paramsObj.outFormat
    )
    if not os.path.exists(paramsObj.viewAngleImg):
        paramsObj.viewAngleImg = None
    if paramsObj.validMaskImage is not None:
        rsgislib.rastergis.pop_rat_img_stats(paramsObj.validMaskImage, True, True)
    if paramsObj.viewAngleImg is not None:
        rsgislib.imageutils.pop_img_stats(paramsObj.viewAngleImg, True, 1000, True)
    print("")
    if paramsObj.validMaskImage is not None:
    ...
    if paramsObj.reproject and (paramsObj.validMaskImage is not None):
        if not paramsObj.pxlResDefd:
            ...
            if validImgDS is None:
                raise ARCSIException(
                    "Could not open the Valid Image Mask " + paramsObj.validMaskImage
                )
            geoTransform = validImgDS.GetGeoTransform()
            if geoTransform is None:
                raise ARCSIException(
                    "Could read the geotransform from the Valid Image Mask "
        ...
        paramsObj.projImgBBOX = paramsObj.sensorClass.getReProjBBOX(
            paramsObj.outWKTFile,
            paramsObj.outProj4File,
            paramsObj.useWKT2Reproject,
            paramsObj.xPxlRes,
            paramsObj.yPxlRes,
            True,
        )
        print(
            "Output Image BBOX (TL, BR): [({0:10.2f}, {1:10.2f}), ({2:10.2f}, {3:10.2f})]".format(
                paramsObj.projImgBBOX["MinX"],
                paramsObj.projImgBBOX["MaxY"],
                paramsObj.projImgBBOX["MaxX"],
                paramsObj.projImgBBOX["MinY"],
            )
        )
        ...
        cmd = (
            ...
            + paramsObj.reProjStr
            + " -tr "
            + str(paramsObj.xPxlRes)
            + " "
            + str(paramsObj.yPxlRes)
            + " -ot Byte -wt Float32 "
            + "-te "
            + str(paramsObj.projImgBBOX["MinX"])
            + " "
            + str(paramsObj.projImgBBOX["MinY"])
            + " "
            + str(paramsObj.projImgBBOX["MaxX"])
            + " "
            + str(paramsObj.projImgBBOX["MaxY"])
        ...
        if not os.path.exists(paramsObj.validMaskImageProj):
            raise ARCSIException(
                "Reprojected valid image mask is not present: "
                + paramsObj.validMaskImageProj
            )
        else:
            rsgislib.rastergis.pop_rat_img_stats(
                paramsObj.validMaskImageProj, True, True
            )
        paramsObj.finalOutFiles["VALID_MASK"] = paramsObj.validMaskImageProj
        print("")

    if paramsObj.reproject and (
        (paramsObj.viewAngleImg != "") and (paramsObj.viewAngleImg is not None)
    ):
        if not paramsObj.pxlResDefd:
            viewAngleImgDS = gdal.Open(paramsObj.viewAngleImg, gdal.GA_ReadOnly)
            if viewAngleImgDS is None:
                raise ARCSIException(
                    "Could not open the Valid Image Mask " + paramsObj.viewAngleImg
                )
        ...
        cmd = (
            ...
            + " "
            + str(paramsObj.projImgBBOX["MaxY"])
            + " -r cubicspline -tap -srcnodata 99999 -dstnodata 99999 -of "
            + paramsObj.outFormat
            + " -overwrite "
            + paramsObj.viewAngleImg
            + " "
        ...
        if not os.path.exists(paramsObj.viewAngleImgProj):
            ...
            rsgislib.imageutils.pop_img_stats(
                paramsObj.viewAngleImgProj, True, 99999, True
            )
        rsgislib.imageutils.delete_gdal_layer(paramsObj.viewAngleImg)
...
def createFootprint(paramsObj):
    if paramsObj.prodsToCalc["FOOTPRINT"]:
        if paramsObj.validMaskImage is None:
            raise ARCSIException(
                "To generate a footprint a valid image mask is required - not supported by this sensor?"
            )
        outFootprintLyrName = ""
        if paramsObj.reproject:
            outFootprintLyrName = paramsObj.outBaseNameProj + "_footprint"
            paramsObj.footprintVecFile = paramsObj.sensorClass.generateImageFootprint(
                paramsObj.validMaskImageProj, paramsObj.outFilePath, outFootprintLyrName
            )
        else:
            outFootprintLyrName = paramsObj.outBaseName + "_footprint"
            paramsObj.footprintVecFile = paramsObj.sensorClass.generateImageFootprint(
                paramsObj.validMaskImage, paramsObj.outFilePath, outFootprintLyrName
            )
...
def createSaturatedImage(paramsObj):
    if paramsObj.prodsToCalc["SATURATE"]:
        ...
        paramsObj.saturateImage = paramsObj.sensorClass.generateImageSaturationMask(
            paramsObj.outFilePath, outName, paramsObj.outFormat
        )

        if paramsObj.reproject:
            outNameProj = paramsObj.outBaseNameProj + "_sat" + paramsObj.outFormatExt
            paramsObj.saturateImageProj = os.path.join(
                paramsObj.outFilePath, outNameProj
            )
            cmd = (
                "gdalwarp -t_srs "
                + paramsObj.reProjStr
            ...
            try:
                ...
                raise ARCSIException(
                    "Could not re-projection saturated image mask: " + cmd
                )
            if not os.path.exists(paramsObj.saturateImageProj):
                raise ARCSIException(
                    "Reprojected saturated image mask is not present: "
                    + paramsObj.saturateImageProj
                )
            else:
                rsgislib.imageutils.delete_gdal_layer(paramsObj.saturateImage)
                paramsObj.saturateImage = paramsObj.saturateImageProj
        if paramsObj.calcStatsPy:
            print("Calculating Statistics...")
            rsgislib.rastergis.pop_rat_img_stats(paramsObj.saturateImage, True, True)
    ...
    if paramsObj.prodsToCalc["RAD"]:
        ...
        if paramsObj.sensorClass.maskInputImages():
            ...
            (
                radianceImageTmp,
                paramsObj.maskImage,
            ) = paramsObj.sensorClass.applyImageDataMask(
                paramsObj.inputHeader,
                paramsObj.radianceImage,
                paramsObj.outFilePath,
                outMaskName,
                outImgName,
            ...
            if radianceImageTmp is not paramsObj.radianceImage:
                rsgislib.imageutils.delete_gdal_layer(paramsObj.radianceImage)
            ...
            if paramsObj.maskImage is not None:
                ...
                if paramsObj.calcStatsPy:
                    print("Calculating Statistics...")
                    rsgislib.imageutils.pop_img_stats(
                        paramsObj.radianceImage, True, 0.0, True
                    )
                    rsgislib.rastergis.pop_rat_img_stats(
                        paramsObj.maskImage, True, True
                    )

        if paramsObj.validMaskImage is not None:
            print("Masking to valid data area.")
            paramsObj.processStageStr = paramsObj.processStageStr + "_vmsk"
            outRadPathName = os.path.join(
                paramsObj.outFilePath,
                paramsObj.outBaseName
                + paramsObj.processStageStr
                + "_rad"
            ...
            rsgislib.imageutils.mask_img(
                ...
                outRadPathName,
                paramsObj.outFormat,
                rsgislib.imageutils.get_rsgislib_datatype_from_img(
                    paramsObj.radianceImage
                ),
                0.0,
                0.0,
            ...
            if paramsObj.thermalRadImage is not None:
                ...
                rsgislib.imageutils.delete_gdal_layer(paramsObj.thermalRadImage)
                paramsObj.thermalRadImage = outThermPathName
            if paramsObj.reproject:
                rsgislib.imageutils.delete_gdal_layer(paramsObj.validMaskImage)
        ...
        if paramsObj.prodsToCalc["SHARP"]:
            ...
            paramsObj.sensorClass.sharpenLowResRadImgBands(
                paramsObj.radianceImage, outRadSharpImage, paramsObj.outFormat
            )
            rsgislib.imageutils.delete_gdal_layer(paramsObj.radianceImage)
        ...
        if paramsObj.reproject:
            if paramsObj.radianceImage is not None:
                ...
                cmd = (
                    ...
                    + str(paramsObj.projImgBBOX["MaxY"])
                    + " -r "
                    + paramsObj.interpAlgor
                    + " -tap -srcnodata 0 -dstnodata 0 -of "
                    + paramsObj.outFormat
                    + " -overwrite "
                    + paramsObj.radianceImage
                    + " "
                ...
                try:
                    ...
                    raise ARCSIException(
                        "Could not re-projection radiance image: " + cmd
                    )
                if not os.path.exists(outRadImagePath):
                    raise ARCSIException(
                        "Reprojected radiance image is not present: " + outRadImagePath
                    )
                else:
                    rsgislib.imageutils.delete_gdal_layer(paramsObj.radianceImage)
            ...
            if paramsObj.thermalRadImage is not None:
                ...
                cmd = (
                    ...
                    + paramsObj.thermalRadImage
                    + " "
                    + outThermRadImagePath
                )
                try:
                    subprocess.call(cmd, shell=True)
                except OSError as e:
                    raise ARCSIException(
                        "Could not re-projection thermal radiance image: " + cmd
                    )
                if not os.path.exists(outThermRadImagePath):
                    raise ARCSIException(
                        "Reprojected thermal radiance image is not present: "
                        + outThermRadImagePath
                    )
                else:
                    rsgislib.imageutils.delete_gdal_layer(paramsObj.thermalRadImage)
        ...
        if paramsObj.calcStatsPy:
            print("Calculating Statistics...")
            rsgislib.imageutils.pop_img_stats(paramsObj.radianceImage, True, 0.0, True)
            if paramsObj.thermalRadImage is not None:
                rsgislib.imageutils.pop_img_stats(
                    paramsObj.thermalRadImage, True, 0.0, True
                )
...
def calcThermalBrightness(paramsObj):
    ...
    if paramsObj.prodsToCalc["THERMAL"]:
        ...
        paramsObj.thermalBrightImage = paramsObj.sensorClass.convertThermalToBrightness(
            paramsObj.thermalRadImage,
            paramsObj.outFilePath,
            outName,
            paramsObj.outFormat,
        ...
        if paramsObj.calcStatsPy:
            print("Calculating Statistics...")
            rsgislib.imageutils.pop_img_stats(
                paramsObj.thermalBrightImage, True, 0.0, True
            )
...
def calcTOAReflectance(paramsObj):
    ...
    if paramsObj.prodsToCalc["TOA"]:
        ...
        paramsObj.toaImage = paramsObj.sensorClass.convertImageToTOARefl(
            paramsObj.radianceImage,
            paramsObj.outFilePath,
            outName,
            paramsObj.outFormat,
        ...
        if not rsgislib.imageutils.do_gdal_layers_have_same_proj(
            paramsObj.radianceImage, paramsObj.toaImage
        ):
            tmpImg = os.path.join(
                paramsObj.tmpPath,
                paramsObj.outBaseName
            ...
            rsgislib.imageutils.rename_gdal_layer(paramsObj.toaImage, tmpImg)
            interpAlgorOpt = paramsObj.interpAlgor
            if paramsObj.interpAlgor == "near":
                interpAlgorOpt = rsgislib.INTERP_NEAREST_NEIGHBOUR
            ...
            rsgislib.imageutils.delete_gdal_layer(tmpImg)

        print("Setting Band Names...")
        paramsObj.sensorClass.setBandNames(paramsObj.toaImage)
        if paramsObj.calcStatsPy:
            print("Calculating Statistics...")
            rsgislib.imageutils.pop_img_stats(paramsObj.toaImage, True, 0.0, True)
...
def performCloudMasking(paramsObj):
    ...
    if paramsObj.prodsToCalc["CLOUDS"]:
        ...
        paramsObj.propOfCloud = rsgislib.imagecalc.calc_prop_true_exp(
            "b1==1?1:b1==2?1:0",
            [rsgislib.imagecalc.BandDefn("b1", paramsObj.cloudsImage, 1)],
            paramsObj.validMaskImage,
        )
        ...
        if paramsObj.propOfCloud < 0.95:  # Less than 95% cloud cover then process.
            ...
            rsgislib.imageutils.mask_img(
                ...
                outputTOAImage,
                paramsObj.outFormat,
                rsgislib.imageutils.get_rsgislib_datatype_from_img(paramsObj.toaImage),
                0,
                [1, 2],
            )
...
def performClearSkyMasking(paramsObj):
    if paramsObj.prodsToCalc["CLEARSKY"]:
        ...
        paramsObj.clearskyImage = paramsObj.sensorClass.generateClearSkyMask(
            paramsObj.cloudsImage,
            paramsObj.validMaskImage,
            paramsObj.outFilePath,
            outName,
            paramsObj.outFormat,
            paramsObj.tmpPath,
            paramsObj.initClearSkyRegionDist,
            paramsObj.initClearSkyRegionMinSize,
            paramsObj.finalClearSkyRegionDist,
            paramsObj.clearSkyMorphSize,
        )
        if paramsObj.calcStatsPy:
            print("Calculating Statistics...")
            rsgislib.rastergis.pop_rat_img_stats(paramsObj.clearskyImage, True, True)
        paramsObj.finalOutFiles["CLEARSKY_MASK"] = paramsObj.clearskyImage

        # Calculate the proportion of the scene which is clear sky.
        paramsObj.propOfClearSky = rsgislib.imagecalc.calc_prop_true_exp(
            "b1==1?1:0",
            [rsgislib.imagecalc.BandDefn("b1", paramsObj.clearskyImage, 1)],
            paramsObj.validMaskImage,
        )
        ...
        if (
            paramsObj.propOfClearSky > 0.05
        ):  # Keep going if at least 5% of the scene is clear sky
...
def prepareDEM(paramsObj):
    if (paramsObj.demFile is not None) and (paramsObj.demFile != ""):
        ...
        outDEMNameTmp = os.path.join(
            paramsObj.outFilePath,
            (paramsObj.outBaseName + "_demtmp" + paramsObj.outFormatExt),
        )
        rsgislib.imageutils.create_copy_img(
            paramsObj.radianceImage,
            outDEMNameTmp,
            1,
            paramsObj.demNoDataVal,
            paramsObj.outFormat,
        ...
        paramsObj.outDEMName = os.path.join(
            paramsObj.outFilePath,
            (paramsObj.outBaseName + "_dem" + paramsObj.outFormatExt),
        )
        ...
        paramsObj.outDEMNameMsk = os.path.join(
            paramsObj.outFilePath,
            (paramsObj.outBaseName + "_demmsk" + paramsObj.outFormatExt),
        )
        if paramsObj.prodsToCalc["CLEARSKY"]:
            rsgislib.imageutils.mask_img(
                paramsObj.outDEMName,
        ...
        if paramsObj.prodsToCalc["CLEARSKY"] or paramsObj.prodsToCalc["CLOUDS"]:
            rsgislib.imageutils.pop_img_stats(
                paramsObj.outDEMNameMsk, True, paramsObj.demNoDataVal, True
            )
        rsgislib.imageutils.pop_img_stats(
            paramsObj.outDEMName, True, paramsObj.demNoDataVal, True
        )

        # Remove tmp DEM file.
        rsgislib.imageutils.delete_gdal_layer(outDEMNameTmp)
...
def calcTopoShadowMask(paramsObj):
    if paramsObj.prodsToCalc["TOPOSHADOW"]:
        ...
        paramsObj.topoShadowImage = paramsObj.sensorClass.generateTopoDirectShadowMask(
            tmpDEMFile,
            paramsObj.outFilePath,
            outName,
            paramsObj.outFormat,
        ...
        if paramsObj.calcStatsPy:
            print("Calculating Statistics...")
            rsgislib.rastergis.pop_rat_img_stats(paramsObj.topoShadowImage, True, True)
...
def performDOS(paramsObj):
    ...
    if paramsObj.prodsToCalc["DOS"]:
        ...
        (
            paramsObj.srefDOSImage,
            offVals,
        ) = paramsObj.sensorClass.convertImageToReflectanceSimpleDarkSubtract(
            paramsObj.toaImage,
            paramsObj.outFilePath,
            outName,
            paramsObj.outFormat,
            paramsObj.dosOutRefl,
        )
        ...
        if paramsObj.calcStatsPy:
            print("Calculating Statistics...")
            rsgislib.imageutils.pop_img_stats(paramsObj.srefDOSImage, True, 0.0, True)
            if paramsObj.fullImgOuts:
                rsgislib.imageutils.pop_img_stats(
                    paramsObj.srefDOSWholeImage, True, 0.0, True
                )
            print("")
...
def estimateSceneAOT(paramsObj):
    ...
    if paramsObj.prodsToCalc["DOSAOTSGL"]:
        ...
        paramsObj.aotVal = paramsObj.sensorClass.estimateSingleAOTFromDOS(
            paramsObj.radianceImage,
            paramsObj.toaImage,
            paramsObj.outDEMName,
            paramsObj.tmpPath,
        ...
        print(
            "AOT Search Range = ["
            + str(paramsObj.minAOT)
            + ", "
            + str(paramsObj.maxAOT)
            + "]"
        )
    ...
    if paramsObj.prodsToCalc["DDVAOT"]:
        ...
        paramsObj.aotFile = paramsObj.sensorClass.estimateImageToAODUsingDDV(
            paramsObj.radianceImage,
            paramsObj.toaImage,
            paramsObj.outDEMName,
            paramsObj.topoShadowImage,
        ...
        if paramsObj.calcStatsPy:
            print("Calculating Statistics...")
            rsgislib.imageutils.pop_img_stats(paramsObj.aotFile, True, 0.0, True)
    ...
    if paramsObj.prodsToCalc["DOSAOT"]:
        ...
        paramsObj.aotFile = paramsObj.sensorClass.estimateImageToAODUsingDOS(
            paramsObj.radianceImage,
            paramsObj.toaImage,
            paramsObj.outDEMName,
            paramsObj.topoShadowImage,
...
def calculateSREF(paramsObj):
    ...
    if paramsObj.prodsToCalc["SREF"]:
        if (paramsObj.prodsToCalc["DDVAOT"] or paramsObj.prodsToCalc["DOSAOT"]) and (
            ...
            if aotVal < 0.01:
                print(
                    "WARNING: Something has gone wrong as AOT value is 0 or below. Setting to 0.05"
                )
                aotVal = 0.05
            imgDS = None

        if (
            (paramsObj.aotVal is None)
            and (paramsObj.visVal is None)
            and (paramsObj.aotFile == "")
        ):
            ...
            paramsObj.aotVal = rsgislib.imagecalibration.visibility_to_aot(
                paramsObj.visVal
            )
        ...
        if paramsObj.demFile is None:
            ...
            if (paramsObj.aotFile is None) or (paramsObj.aotFile == ""):
                ...
                ) = paramsObj.sensorClass.convertImageToSurfaceReflDEMElevLUT(
                    paramsObj.radianceImage,
                    paramsObj.outDEMName,
                    paramsObj.outFilePath,
                    outName,
                ...
                if paramsObj.minAOT < 0.01:
                    minAOT = 0.05
                paramsObj.maxAOT = arcsilib.arcsiutils.findMaximumAOT(statsAOT[1])
                ...
                (
                    paramsObj.srefImage,
                    paramsObj.sixsLUTCoeffs,
                ) = paramsObj.sensorClass.convertImageToSurfaceReflAOTDEMElevLUT(
                    paramsObj.radianceImage,
                    paramsObj.outDEMName,
                    paramsObj.aotFile,
                    paramsObj.outFilePath,
        ...
        if paramsObj.calcStatsPy:
            ...
            if paramsObj.fullImgOuts:
                rsgislib.imageutils.pop_img_stats(
                    paramsObj.sref6SWholeImage, True, 0.0, True
                )
...
def calculateStandarisedSREF(paramsObj):
    if paramsObj.prodsToCalc["STDSREF"]:
        if paramsObj.sixsLUTCoeffs is None:
            raise ARCSIException(
                "To calculate standardised reflectance 'STDSREF' you need to have a LUT within 6S.\nTherefore, a DEM is required and optional an AOT surface should be calculated."
            )
        ...
        (
            paramsObj.stdSREFImg,
            paramsObj.stdSREFWholeImg,
        ) = paramsObj.sensorClass.convertSREF2StdisedSREF(
            paramsObj.srefImage,
            paramsObj.sref6SWholeImage,
            paramsObj.outDEMName,
            paramsObj.topoShadowImage,
        ...
        if paramsObj.calcStatsPy:
            print("Calculating Statistics...")
            rsgislib.imageutils.pop_img_stats(paramsObj.stdSREFImg, True, 0.0, True)
            if paramsObj.fullImgOuts:
                rsgislib.imageutils.pop_img_stats(
                    paramsObj.stdSREFWholeImg, True, 0.0, True
                )
...
def exportMetaData(paramsObj):
    if paramsObj.prodsToCalc["METADATA"]:
        ...
        paramsObj.sensorClass.generateMetaDataFile(
            paramsObj.outFilePath,
            outName,
            paramsObj.productsStr,
            validMaskImagePath,
            paramsObj.prodsToCalc["FOOTPRINT"],
            paramsObj.calcdOutVals,
            paramsObj.finalOutFiles,
        )
    ...
    try:
        ...
        if paramsObj is not None:
            ...
            if len(failedProdsList) > 0:
                print(
                    "Error: The following products were not generated:", file=sys.stderr
                )
                print(" ".join(failedProdsList), file=sys.stderr)
                print("Input Header: '" + inputHeader + "'", file=sys.stderr)
                if paramsObj.outBaseName is not None:
                    print(
                        "Output Basename: '" + paramsObj.outBaseName + "'",
                        file=sys.stderr,
                    )
...
def print2ConsoleListSensors():
    ...
    print(
        "\tLandsat 1 MSS | 'ls1'       | RAD, TOA, DOSAOT, DOSAOTSGL, SREF, STDSREF, DOS, TOPOSHADOW, FOOTPRINT, METADATA"
    )
    print(
        "\tLandsat 2 MSS | 'ls2'       | RAD, TOA, DOSAOT, DOSAOTSGL, SREF, STDSREF, DOS, TOPOSHADOW, FOOTPRINT, METADATA"
    )
    print(
        "\tLandsat 3 MSS | 'ls3'       | RAD, TOA, DOSAOT, DOSAOTSGL, SREF, STDSREF, DOS, TOPOSHADOW, FOOTPRINT, METADATA"
    )
    print(
        "\tLandsat 4 MSS | 'ls4mss'    | RAD, TOA, DOSAOT, DOSAOTSGL, SREF, STDSREF, DOS, TOPOSHADOW, FOOTPRINT, METADATA"
    )
    print(
        "\tLandsat 4 TM  | 'ls4tm'     | RAD, TOA, DOSAOT, DDVAOT, DOSAOTSGL, STDSREF, SREF, DOS, THERMAL, TOPOSHADOW, FOOTPRINT, METADATA"
    )
    print(
        "\tLandsat 5 MSS | 'ls5mss'    | RAD, TOA, DOSAOT, DOSAOTSGL, SREF, STDSREF, DOS, TOPOSHADOW, FOOTPRINT, METADATA"
    )
    print(
        "\tLandsat 5 TM  | 'ls5tm'     | RAD, TOA, DOSAOT, DDVAOT, DOSAOTSGL, STDSREF, SREF, DOS, THERMAL, TOPOSHADOW, FOOTPRINT, METADATA"
    )
    print(
        "\tLandsat 7 ETM | 'ls7'       | RAD, TOA, DOSAOT, DDVAOT, DOSAOTSGL, STDSREF, SREF, DOS, THERMAL, TOPOSHADOW, FOOTPRINT, METADATA"
    )
    print(
        "\tLandsat 8     | 'ls8'       | RAD, TOA, DOSAOT, DDVAOT, DOSAOTSGL, STDSREF, SREF, DOS, THERMAL, TOPOSHADOW, FOOTPRINT, METADATA"
    )
    print(
        "\tRapideye      | 'rapideye'  | RAD, TOA, DOSAOT, DOSAOTSGL, SREF, STDSREF, DOS, TOPOSHADOW, METADATA"
    )
    print(
        "\tWorldView2    | 'wv2'       | RAD, TOA, DOSAOT, DOSAOTSGL, SREF, STDSREF, DOS, TOPOSHADOW, METADATA"
    )
    print(
        "\tSPOT5         | 'spot5'     | RAD, TOA, DOSAOT, DOSAOTSGL, SREF, STDSREF, DOS, TOPOSHADOW, METADATA"
    )
    print(
        "\t----------------------------------------------------------------------------------------------------------------------------------"
    )
